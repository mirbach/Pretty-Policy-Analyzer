...
import { Search, CheckSquare, Square, Shield, Monitor, User } from 'lucide-react';
...
export function GPOList({ selectedId, compareIds, onSelect, onCompareToggle, onSelectAll }: GPOListProps) {
  ...
  const visibleIds = gpos?.map((g) => g.id) ?? [];
  const allSelected = visibleIds.length > 0 && visibleIds.every((id) => compareIds.includes(id));

  const handleSelectAll = () => {
    if (allSelected) {
      // deselect all visible
      onSelectAll(compareIds.filter((id) => !visibleIds.includes(id)));
    } else {
      // add all visible to existing selection
      const merged = Array.from(new Set([...compareIds, ...visibleIds]));
      onSelectAll(merged);
    }
  };
